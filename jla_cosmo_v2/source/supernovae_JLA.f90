    ...
    type, extends(CosmologyLikelihood) :: JLALikelihood
    contains
    ...
    TYPE, PRIVATE :: supernova
        ...
        REAL(dl) :: ra            ! Right ascension (deg.)
        REAL(dl) :: dec           ! Declination (deg.)
        REAL(dl) :: malmcor       ! Size of the bias correction (mag, already applied to msb)
    END TYPE supernova
    ...
    PRIVATE :: count_lines, read_jla_lc_data, get_free_lun, read_cov_matrix
    ...
    character (LEN=Ini_max_string_len):: jla_filename
    integer alpha_i, beta_i

    if (.not. Ini_Read_Logical_File(Ini, 'use_JLA',.false.)) return
    ...
    JLA_marginalize = Ini_Read_Logical_File(Ini, 'JLA_marginalize',.false.)
    if (JLA_marginalize) then
       WRITE(*,*) 'WE have a problem: should never get here.'
        JLA_marge_steps = Ini_Read_int_File(Ini,'JLA_marge_steps',5)
        JLA_step_width = Ini_read_Double_File(ini,'JLA_step_width',0.05d0)
        JLA_int_points=0
    ...
    jla_filename = Ini_Read_String_Default_File(Ini,'jla_dataset',trim(DataDir)//'jla.dataset')
...
500 WRITE (*,*) 'Failed to open cov matrix file ' // TRIM(filename)
    ...
    DO i=1,nlines
        ...
        !We have a few formats to try.  First, there is the very long
        ! format with thirdvar and dataset, ra, dec and bias
        ! correction.  If that fails, try without data set.  If that
        ! fails, try without thirdvar but with dataset, and finally
        ! with neither
        ...
        sndata(count)%dataset, sndata(count)%ra, sndata(count)%dec,&
        sndata(count)%malmcor
...
140 WRITE(*,*) 'Error reading in input data with: ',inline
    ...
    IF (bad_ini_open) THEN
        WRITE(*,*) 'Error opening JLA dataset file ' // TRIM(filename)
        STOP
    ENDIF

    name = Ini_Read_String_File(Ini, 'name', .FALSE. )
    data_file = Ini_Read_String_Default_File(Ini,'data_file',trim(DataDir)//'jla_lcparams.txt')
